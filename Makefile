...
pkgdatadir = $(datadir)/opalcoin
pkgincludedir = $(includedir)/opalcoin
pkglibdir = $(libdir)/opalcoin
pkglibexecdir = $(libexecdir)/opalcoin
...
CONFIG_HEADER = $(top_builddir)/src/config/opalcoin-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/opalcoin-config.h.in \
...
ACLOCAL = ${SHELL} /root/opal/auop-master/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /root/opal/auop-master/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/opal/auop-master/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/opal/auop-master/build-aux/missing automake-1.15
AVX2_CXXFLAGS = -mavx -mavx2
AWK = mawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = opalcoin-cli
BITCOIN_DAEMON_NAME = opalcoind
BITCOIN_GUI_NAME = opalcoin-qt
BITCOIN_TX_NAME = opalcoin-tx
...
MAINT = 
MAKEINFO = ${SHELL} /root/opal/auop-master/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = opalcoin
PACKAGE_BUGREPORT = https://github.com/opalcoinnetnet/opalcoin/issues
PACKAGE_NAME = Auop Coin
PACKAGE_STRING = Auop Coin 2.0.0
PACKAGE_TARNAME = opalcoin
PACKAGE_URL = https://opalcoin.net/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/opal/auop-master
abs_srcdir = /root/opal/auop-master
abs_top_builddir = /root/opal/auop-master
abs_top_srcdir = /root/opal/auop-master
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/opal/auop-master/build-aux/install-sh
...
DIST_CONTRIB = $(top_srcdir)/contrib/opalcoin-cli.bash-completion \
	       $(top_srcdir)/contrib/opalcoin-tx.bash-completion \
	       $(top_srcdir)/contrib/opalcoind.bash-completion \
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/opalcoin.ico \
  $(top_srcdir)/share/pixmaps/nsis-header.bmp \
  $(top_srcdir)/share/pixmaps/nsis-wizard.bmp \
  $(top_srcdir)/doc/README_windows.txt

LINUX_PACKAGING = $(top_srcdir)/share/pixmaps/opalcoin16.xpm \
  $(top_srcdir)/share/pixmaps/opalcoin32.xpm \
  $(top_srcdir)/share/pixmaps/opalcoin128.png
...
COVERAGE_INFO = baseline.info \
  test_opalcoin_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_opalcoin_coverage.info test_opalcoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/OPALCOIN-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/opalcoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/opalcoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/opalcoin-config.h
$(top_srcdir)/src/config/opalcoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/opalcoin-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/OPALCOIN-Qt: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/OPALCOIN-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/OPALCOIN-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_opalcoin.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_opalcoin -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_opalcoin_filtered.info: test_opalcoin.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_opalcoin_filtered.info
...
#test_opalcoin_coverage.info: baseline_filtered.info test_opalcoin_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_opalcoin_filtered.info -o $@

#total_coverage.info: test_opalcoin_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_opalcoin_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_opalcoin.coverage/.dirstamp:  test_opalcoin_coverage.info
...
#cov: test_opalcoin.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_opalcoin.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
