// Copyright (c) 2018-2019 The OPALCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zauop/deterministicmint.h>
#include "zauoptracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzAUOPTracker::CzAUOPTracker(std::string strWalletFile)
{
...
CzAUOPTracker::~CzAUOPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAUOPTracker::Init()
{
...
bool CzAUOPTracker::Archive(CMintMeta& meta)
{
...
bool CzAUOPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAUOPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAUOPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAUOPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzAUOPTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzAUOPTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzAUOPTracker::GetSerialHashes()
{
...
CAmount CzAUOPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAUOPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAUOPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAUOPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAUOPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAUOPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAUOPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAUOPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAUOPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAUOPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAUOPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzAUOPWallet* zAUOPWallet)
{
    bool iszAUOPWalletInitialized = (NULL != zAUOPWallet);
    CMintMeta meta;
    ...
    if (! iszAUOPWalletInitialized)
        zAUOPWallet = new CzAUOPWallet(strWalletFile);
    meta.isSeedCorrect = zAUOPWallet->CheckSeed(dMint);
    if (! iszAUOPWalletInitialized)
        delete zAUOPWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzAUOPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAUOPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAUOPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAUOPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAUOPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAUOPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzAUOPWallet* zAUOPWallet = new CzAUOPWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zAUOPWallet);
        }
        delete zAUOPWallet;
        LogPrint("zero", "%s: added %d dzauop from DB\n", __func__, listDeterministicDB.size());
    }
