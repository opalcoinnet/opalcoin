...
CzAUOPWallet::CzAUOPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zauop seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAUOPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAUOPSeed_deprecated()) {
                    LogPrintf("%s: Updated zAUOP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zauop seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zauop wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAUOPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAUOPCount(nCountLastUsed);
    else if (!walletdb.ReadZAUOPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAUOPWallet::Lock()
{
    seedMaster = 0;
}

void CzAUOPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAUOPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAUOP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAUOPWallet::LoadMintPoolFromDB()
{
...
void CzAUOPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAUOPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAUOPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zauopTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAUOPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAUOP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zauopTracker which also adds to database
    pwalletMain->zauopTracker->Add(dMint, true);
...
void CzAUOPWallet::SeedToZAUOP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAUOPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAUOPWallet::GenerateDeterministicZAUOP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAUOPWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAUOPWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzAUOPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
