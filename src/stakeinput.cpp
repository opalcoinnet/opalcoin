...
CZAUOPStake::CZAUOPStake(const libzerocoin::CoinSpend& spend)
{
...
int CZAUOPStake::GetChecksumHeightFromMint()
{
...
int CZAUOPStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZAUOPStake::GetChecksum()
{
    return nChecksum;
}

// The zAUOP block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZAUOPStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZAUOPStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zAUOP staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZAUOPStake::GetUniqueness()
{
    //The unique identifier for a zAUOP is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZAUOPStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZAUOPStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zAUOP that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZAUOPOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAUOP output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zAUOP", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZAUOPOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zAUOP output", __func__);
...
bool CZAUOPStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZAUOPStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzAUOPTracker* zauopTracker = pwallet->zauopTracker.get();
    CMintMeta meta;
    if (!zauopTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zauopTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!AUOP Stake
bool CAUOPStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CAUOPStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CAUOPStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CAUOPStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CAUOPStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CAUOPStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CAUOPStake::GetUniqueness()
{
    //The unique identifier for a AUOP stake is the outpoint
...
CBlockIndex* CAUOPStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
