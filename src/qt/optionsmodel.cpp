...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::AUOP);
    ...
    if (settings.contains("nAnonymizeAUOPcoinAmount") || reset)
        SoftSetArg("-anonymizeopalcoinamount", settings.value("nAnonymizeAUOPcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in opalcoin.cpp during shotdown to also remove the window positions
