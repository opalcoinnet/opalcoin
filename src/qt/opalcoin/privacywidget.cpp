...
#include "qt/opalcoin/privacywidget.h"
#include "qt/opalcoin/forms/ui_privacywidget.h"
#include "qt/opalcoin/qtutils.h"
#include "guiutil.h"
#include "qt/opalcoin/denomgenerationdialog.h"
#include "qt/opalcoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(OPALCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zAUOP anonymizes your AUOP by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zAUOP or convert back to AUOP"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 AUOP ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zAUOP");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zAUOP");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zAUOP");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zAUOP");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zAUOP");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zAUOP");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zAUOP");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zAUOP");
    ...
    // Only Convert to AUOP enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzAUOP->setTitleClassAndText("btn-title-grey", "Total 0 zAUOP");
    ui->btnTotalzAUOP->setSubTitleClassAndText("text-subtitle", "Show denominations of zAUOP owned.");
    ui->btnTotalzAUOP->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select AUOP outputs to mint into zAUOP.");
    ...
    connect(ui->btnTotalzAUOP, SIGNAL(clicked()), this, SLOT(onTotalZauopClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zAUOP");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of AUOP to mint into zAUOP"));
    }else{
        btnText = tr("Convert back to AUOP");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zAUOP to convert back into AUOP"));
    }
...
void PrivacyWidget::onTotalZauopClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzAUOP->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzAUOP->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zAUOP is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zAUOP, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zAUOP").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zAUOP minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZauop(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zAUOP converted back to AUOP"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAUOP </b>";
    ...
    ui->btnTotalzAUOP->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
