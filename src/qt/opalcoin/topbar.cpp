...
#include "qt/opalcoin/topbar.h"
#include "qt/opalcoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/opalcoin/lockunlock.h"
#include "qt/opalcoin/qtutils.h"
#include "qt/opalcoin/receivedialog.h"
...
TopBar::TopBar(OPALCOINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopAUOP, ui->labelAmountTopzAUOP}, "amount-small-topbar");
    setCssProperty({ui->labelAmountAUOP, ui->labelAmountzAUOP}, "amount-topbar");
    setCssProperty({ui->labelPendingAUOP, ui->labelPendingzAUOP, ui->labelImmatureAUOP, ui->labelImmaturezAUOP}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzAUOP->setVisible(false);
    ui->labelAmountzAUOP->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzAUOP->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezAUOP->setVisible(false);
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("OPALCOIN")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount auopAvailableBalance = balance + delegatedBalance;

    // zAUOP Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalAUOP = GUIUtil::formatBalance(auopAvailableBalance, nDisplayUnit);
    QString totalzAUOP = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopAUOP->setText(totalAUOP);
    ui->labelAmountTopzAUOP->setText(totalzAUOP);

    // Expanded
    ui->labelAmountAUOP->setText(totalAUOP);
    ui->labelAmountzAUOP->setText(totalzAUOP);

    ui->labelPendingAUOP->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzAUOP->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureAUOP->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezAUOP->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
