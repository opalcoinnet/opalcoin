...
#include "qt/opalcoin/dashboardwidget.h"
#include "qt/opalcoin/forms/ui_dashboardwidget.h"
#include "qt/opalcoin/sendconfirmdialog.h"
#include "qt/opalcoin/txrow.h"
...
DashboardWidget::DashboardWidget(OPALCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of AUOP staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareAUOP, "square-chart-auop");
    setCssProperty(ui->labelSquarezAUOP, "square-chart-zauop");
    setCssProperty(ui->labelAUOP, "text-chart-auop");
    setCssProperty(ui->labelZauop, "text-chart-zauop");
    ui->labelZauop->setVisible(false);
    ui->labelSquarezAUOP->setVisible(false);
    ...
    ui->labelAmountZauop->setText("0 zAUOP");
    ui->labelAmountZauop->setVisible(false);
    ui->labelAmountAUOP->setText("0 AUOP");
    setCssProperty(ui->labelAmountAUOP, "text-stake-auop-disable");
    setCssProperty(ui->labelAmountZauop, "text-stake-zauop-disable");
    ...
    ui->btnHowTo->setText(tr("How to get AUOP"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("AUOP")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isAUOP = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZAUOP;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair AUOP, zAUOP
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            auop = (pair.first != 0) ? pair.first / 100000000 : 0;
            zauop = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalAUOP += pair.first;
            chartData->totalZauop += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesAUOP.append(auop);
        chartData->valueszAUOP.append(zauop);

        int max = std::max(auop, zauop);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("AUOP");
    set1 = new QBarSet("zAUOP");
    ...
    set0->append(chartData->valuesAUOP);
    set1->append(chartData->valueszAUOP);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalAUOP > 0 || chartData->totalZauop > 0) {
        setCssProperty(ui->labelAmountAUOP, "text-stake-auop");
        setCssProperty(ui->labelAmountZauop, "text-stake-zauop");
    } else {
        setCssProperty(ui->labelAmountAUOP, "text-stake-auop-disable");
        setCssProperty(ui->labelAmountZauop, "text-stake-zauop-disable");
    }
    forceUpdateStyle({ui->labelAmountAUOP, ui->labelAmountZauop});
    ui->labelAmountAUOP->setText(GUIUtil::formatBalance(chartData->totalAUOP, nDisplayUnit));
    ui->labelAmountZauop->setText(GUIUtil::formatBalance(chartData->totalZauop, nDisplayUnit, true));
