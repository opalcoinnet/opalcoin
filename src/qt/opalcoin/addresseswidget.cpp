...
#include "qt/opalcoin/forms/ui_addresseswidget.h"
#include "qt/opalcoin/addresslabelrow.h"
#include "qt/opalcoin/addnewaddressdialog.h"
#include "qt/opalcoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(OPALCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a OPALCOIN address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress auopAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(auopAdd.Get(), label.toUtf8().constData(),
                auopAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
