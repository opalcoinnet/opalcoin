...
#include "ui_zauopcontroldialog.h"
...
std::set<std::string> ZAUOPControlDialog::setSelectedMints;
std::set<CMintMeta> ZAUOPControlDialog::setMints;

bool CZAUOPControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZAUOPControlDialog::COLUMN_DENOMINATION || column == ZAUOPControlDialog::COLUMN_VERSION || column == ZAUOPControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZAUOPControlDialog::ZAUOPControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZAUOPControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zAUOP Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZAUOP->setProperty("cssClass", "text-main-purple");
    ui->labelZAUOP_int->setProperty("cssClass", "text-main-purple");
...
ZAUOPControlDialog::~ZAUOPControlDialog()
{
    delete ui;
}

void ZAUOPControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAUOPControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZAUOPControlWidgetItem* itemDenom(new CZAUOPControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZAUOPControlWidgetItem *itemMint = new CZAUOPControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zauopTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zauopTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zAUOP.";
            else if (!mint.isSeedCorrect)
                strReason = "The zAUOP seed used to mint this zAUOP is not the same as currently hold in the wallet";
            else
...
void ZAUOPControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAUOPControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZAUOP_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZAUOPControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZAUOPControlDialog::GetSelectedMints()
{
...
void ZAUOPControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
