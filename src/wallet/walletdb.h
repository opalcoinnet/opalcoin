...
public:
    ...
    bool WriteZAUOPSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZAUOPSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZAUOPSeed_deprecated(uint256& seed);
    bool EraseZAUOPSeed();
    bool EraseZAUOPSeed_deprecated();

    bool WriteZAUOPCount(const uint32_t& nCount);
    bool ReadZAUOPCount(uint32_t& nCount);
