...
The build script we'll be using is contained in the OPALCOIN github repository ([contrib/gitian-build.py](https://github.com/opalcoinnet/opalcoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the OPALCOIN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/OPALcoin/OPALCOIN/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, opalcoin-detached-sigs, and opalcoin GitHub repos.
