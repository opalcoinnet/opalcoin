...
*Setup instructions for a gitian build of Auop Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the OPALCOIN
...
to the OPALCOIN GitHub release page.
...
- [Building Auop Coin](#building-opalcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/opalcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/opalcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/opalcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/opalcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/opalcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/opalcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/opalcoinnetnet/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@opalcoin.net) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/opalcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/opalcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/opalcoin-osx-*-build.assert
```
